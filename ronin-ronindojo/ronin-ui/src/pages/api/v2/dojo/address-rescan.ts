...
import { addressRescan, AddressRescanResponse } from "../../../../lib/server/dojoApi";
import { withSessionApi } from "../../../../lib/server/session";

const mockData: AddressRescanResponse = {
  status: "ok",
};
...
interface Response extends AddressRescanResponse {}
...
const handler = (req: NextApiRequest, res: NextApiResponse<Response | ErrorResponse>): Promise<void> => {
  return pipe(
    ...
    taskEither.chain(({ address }) =>
      pipe(
        useRealData
          ? pipe(
              addressRescan(address),
              taskEither.map((addresRescanResponse) => addresRescanResponse.data),
            )
